...
int main(int argc, char* argv[]) {
    ...
    TFile* out = TFile::Open("obelix.root", "RECREATE");
    gDirectory->pwd();
    ...
    double OBLX = 50e-3 / X0_Si + 50e-3 / X0_Kapton;
    // The intrinsic resolution has been measured to be around 3.4um:
    double RES = 9e-3;

    // OBX  OBX  OBX      DUT      OBX  OBX  OBX
    //  |    |    |        |        |    |    |
    //  |    |    |        |        |    |    |
    //  |<-->|    |<------>|<------>|    |    |
    ...
    for(double dist = 20; dist < 151; dist += 1.) {
        ...
        for(int i = 0; i < 3; i++) {
            planes.emplace_back(position, OBLX, true, RES);
            position += dist;
        }
    ...
    c1->cd();
    resolution->SetTitle(
        "OBELIX Track Resolution at DUT;OBELIX plane distance #left[mm#right];resolution at DUT #left[#mum#right]");
    resolution->GetYaxis()->SetRangeUser(0, 9);
